...
const DashboardWidget: FC<DashboardWidgetProps> = ({ className, user }) => {
  ...
  //         id: crypto.randomUUID(),
  //         typeName: 'widgetTool',
  //         description: `create button action stake 0.1 near to ${user?.id}`,
  //         prompt: 'create button action stake 0.1 near to 0x123123',
  //         code: `(props) => {
  //             return (
  //                 <a href={'/chat?prompt=stake 0.1 near to ${user?.id} &widgetId=' + props.widgetId} className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
  //                     stake 0.1 near to ${user?.id}
  //                 </a>
  //             )
  //         }`,
  //         toolWidget: [toolIds],
